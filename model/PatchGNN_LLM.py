...
# ============== 强制依赖检查 ==============
try:
    # 使用 AutoModel 更稳健，它能自动适配 Llama/Qwen 等结构
...
class LlamaAdapter(nn.Module):
    """
    终极修复版 Llama 适配器：
    1. 修正了 LoRA 与 Layer Unfreezing 的执行顺序（确保解冻生效）。
    2. 增强了对不同模型架构（PeftModel/AutoModel）层属性的查找鲁棒性。
    3. 强制单卡模式，避免 DDP 冲突。
    """
    def __init__(self,
                 ...
                 finetune_last_n_layers: int = 0, # 新增参数：显式控制微调层数
                 **kwargs
                 ):
        super().__init__()
        
        # === 1. 强制单卡模式 ===
        # 避免 transformers 的 device_map='auto' 自动切分导致和 GNN 前端不在一个卡上
        device_map = None 
        
        print(f"-------- Configuring LlamaAdapter (Backbone: {model_name_or_path}) --------")
        ...
        # 初始化投影层 (Time Series Dim -> LLM Dim)
        ...
        try:
            self.llm = AutoModel.from_pretrained(
                ...
                # 强制使用 eager 模式，避免 SDPA (Flash Attn) 有时不支持特定 hook 或 hidden states 提取
                attn_implementation="eager" 
            )
        except Exception as e:
            raise RuntimeError(f"[Fatal] 模型加载失败: {e}")

        # === 3. 梯度检查点 (显存优化) ===
        if enable_grad_ckpt:
            self.llm.gradient_checkpointing_enable()
            # 开启输入梯度，这是 LoRA 和 Grad Checkpointing 配合所必须的
            self.llm.enable_input_require_grads()

        # === 4. 应用 LoRA (优先应用，这会冻结原模型) ===
        if lora_r > 0:
            print(f"   >>> Applying LoRA (r={lora_r}, alpha={lora_alpha})...")
            lora_config = LoraConfig(
                task_type=TaskType.FEATURE_EXTRACTION, # 或 CAUSAL_LM
                r=lora_r,
            ...
            # 此时，self.llm 变成了一个 PeftModel，原模型在 self.llm.base_model.model
        
        # === 5. 解冻最后 N 层 (混合微调策略) ===
        # 注意：这必须放在 LoRA 之后，否则 get_peft_model 会重新把它们冻结
        if finetune_last_n_layers > 0:
            self._unfreeze_last_n_layers(finetune_last_n_layers)

        # 打印最终的可训练参数情况
        self.print_trainable_parameters()
        print("-------- LlamaAdapter Ready --------")

    def _unfreeze_last_n_layers(self, n_layers):
        """
        通用解冻逻辑，兼容 PeftModel 和 原始 Model
        """
        # 1. 寻找 layers 列表
        # 这是一个从外向内剥洋葱的过程
        model_ref = self.llm
        ...
        # 路径尝试列表 (针对不同架构封装)
        possible_paths = [
            ["base_model", "model", "layers"],     # PeftModel -> Llama
            ["base_model", "model", "model", "layers"], # 某些深层封装
            ["model", "layers"],                   # AutoModel (Llama)
            ["layers"],                            # LlamaModel directly
        ...
        if layers is not None and isinstance(layers, (list, torch.nn.ModuleList)):
            ...
            print(f"   >>> [Fine-tune] Unfreezing the last {n_layers} / {total_layers} layers...")
            for i in range(total_layers - n_layers, total_layers):
                for param in layers[i].parameters():
                    param.requires_grad = True
        else:
            print("   [Warning] Could not locate 'layers' attribute. Unfreezing skipped.")
    ...
    def forward(self, tokens, attn_mask, idx_ranges):
        ...
        # 4. 前向传播
        # 注意：如果是 PeftModel，它会自动转发给 base model
        out = self.llm(
        ...
        # 5. 根据索引范围提取 Query 部分 (Future Prediction)
...
class LLM4GNN(nn.Module):
    def __init__(self, args, supports=None):
        ...
        self.llm_name = getattr(args, "llm_name", "/home/dragonfly/LLMs/Llama-3.2-1B")
        ...
        if self.use_llm:
            ...
            # 严格模式初始化 LlamaAdapter
            self.llama_adapter = LlamaAdapter(
                model_name_or_path=self.llm_name,
                d_model=2048,                  # 这里的默认值不重要，会被自动覆盖
                ts_dim=self.hid_dim,           # 必须传入 args.hid_dim (例如 64)
                ...
                use_fallback=False,            # 关键：禁用 Fallback
                freeze_backbone=False,
                enable_grad_ckpt=True
            )
            self.reg_head = RegressionHead(
                d_in=self.llama_adapter.llm_hidden_size, # 动态获取真实维度
    ...
    def forecasting(self, time_steps_to_predict, X, truth_time_steps, mask=None, return_aux_losses=False):
        ...
        q_hidden = self.llama_adapter(tokens, attn_mask, idx_ranges)
        ...
        h_final_up = F.linear(h_final, self.llama_adapter.input_proj.weight, self.llama_adapter.input_proj.bias)
