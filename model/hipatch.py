...
class Hi_Patch(nn.Module):
    ...
    def IMTS_Model(self, x, mask_X, x_time):
        ...
        # ===== SAFE graph construction (replace your "Generate graph structure" block) =====
        cur_mask = rearrange(mask_X, 'b n m l c -> b m (n l) c')
        B, M, NL, _ = cur_mask.shape

        mask2 = cur_mask.reshape(B * M, NL, 1).contiguous()
        mask2_cpu = mask2.float().cpu()
        adj_cpu = (mask2_cpu @ mask2_cpu.transpose(1, 2))  
        adj_cpu = (adj_cpu > 0.5)
        ...
        assert source_nodes.min().item() >= 0 and source_nodes.max().item() < num_nodes
        assert target_nodes.min().item() >= 0 and target_nodes.max().item() < num_nodes

        edge_index = torch.stack([source_nodes, target_nodes], dim=0)

        cur_x_time = rearrange(x_time, 'b n m l c -> (b m n l) c').contiguous()
        cur_variable_indices = torch.arange(x.shape[1], device=device).view(1, x.shape[1], 1, 1, 1)
        cur_variable_indices = rearrange(
        cur_variable_indices.expand(B, x.shape[1], M, x.shape[3], 1),
        'b n m l c -> (b m n l) c'
        ).contiguous()
