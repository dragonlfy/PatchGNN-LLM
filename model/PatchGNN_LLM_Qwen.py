...
class QwenAdapter(nn.Module):
    ...
    2. 自动检测 hidden_size (Qwen-0.6B 通常为 896 或 1024)
    3. 支持 LoRA
    """
    def __init__(self,
        ...
        # === 强制配置 ===
        # 不要用 auto 切分，否则会和 model.to() 冲突，尤其是小模型单卡训练时
        device_map = None 
        
        print(f"-------- Configuring QwenAdapter (Path: {model_name_or_path}) --------")
        ...
        # Qwen-0.5B/0.6B usually has hidden_size 896 or 1024. Read from config.
        self.llm_hidden_size = getattr(config, "hidden_size", 896)
        print(f"   >>> Detected Qwen Hidden Size: {self.llm_hidden_size}")
        ...
        try:
            ...
            raise RuntimeError(f"[Fatal] Qwen 模型加载失败: {e}")
...
class LLM4GNN(nn.Module):
    def __init__(self, args, supports=None):
        ...
        # 修改：默认路径为 Qwen
        self.llm_name = getattr(args, "llm_name", "/home/dragonfly/LLMs/Qwen3-0.6B")
        ...
        if self.use_llm:
            ...
            self.qwen_adapter = QwenAdapter(
                ...
                # d_model=896, # 这个参数会被 QwenAdapter 内部的 config 读取覆盖，传什么都行
                ts_dim=self.hid_dim,           
                lora_r=self.lora_r, 
            ...
            self.reg_head = RegressionHead(
                d_in=self.qwen_adapter.llm_hidden_size, # 动态获取真实维度
    ...
    def IMTS_Model(self, x, mask_X, x_time):
        ...
        for n_layer in range(1, self.patch_layer):
            ...
            if T_ > 1 and T_ % 2 != 0:
                ...
                x_time = torch.cat([x_time, torch.zeros(size=[B_, N_, 1, 1]).to(x.device)], dim=2)
                T_ = T_ + 1
    ...
    def forecasting(self, time_steps_to_predict, X, truth_time_steps, mask=None, return_aux_losses=False):
        ...
        q_hidden = self.qwen_adapter(tokens, attn_mask, idx_ranges)
        ...
        h_final_up = F.linear(h_final, self.qwen_adapter.input_proj.weight, self.qwen_adapter.input_proj.bias)
