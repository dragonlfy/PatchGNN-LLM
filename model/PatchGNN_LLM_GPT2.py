...
class GPT2Adapter(nn.Module):
    """
    GPT-2 适配器 (修复版)：
    1. 强制禁用 device_map='auto'
    2. 自动检测 hidden_size
    3. 自动搜索 LoRA 目标层，解决 "Target modules not found" 问题
    """
    def __init__(self,
        ...
        print(f"-------- Configuring GPT2Adapter (Path: {model_name_or_path}) --------")
        ...
        self.llm_hidden_size = getattr(config, "n_embd", getattr(config, "hidden_size", 768))
        print(f"   >>> Detected GPT2 Hidden Size: {self.llm_hidden_size}")
        ...
        try:
            ...
            raise RuntimeError(f"[Fatal] GPT2 模型加载失败: {e}")
        ...
        if _HAS_PEFT:
            # === 自动寻找目标层 ===
            target_modules = set()
            print("   >>> Detecting LoRA target modules...")
            
            # 遍历模型所有层，寻找类似 c_attn, c_proj 的层
            for name, module in self.llm.named_modules():
                # GPT-2 的关键层通常是 Conv1D 或 Linear，名字包含 attn, proj, fc
                if any(k in name for k in ["c_attn", "c_proj", "c_fc", "q_proj", "v_proj"]):
                    # 提取模块的最后一级名称，例如 h.0.attn.c_attn -> c_attn
                    suffix = name.split(".")[-1]
                    target_modules.add(suffix)
            
            target_modules = list(target_modules)
            if len(target_modules) == 0:
                print("   [Warning] No specific GPT-2 modules found (c_attn/c_proj). Trying standard Linear mapping...")
                # 兜底：如果是某种变体 GPT2，可能用了标准 Linear 命名
                target_modules = ["linear", "out_proj"] 
            
            print(f"   >>> LoRA Targets set to: {target_modules}")

            lora_config = LoraConfig(
                task_type=TaskType.FEATURE_EXTRACTION,
                r=lora_r,
                lora_alpha=lora_alpha,
                target_modules=target_modules, # 使用自动检测到的列表
    ...
    def forward(self, tokens, attn_mask, idx_ranges):
        ...
        try:
            # 尝试获取第一层 Block 的参数
            if hasattr(self.llm, "h"):
                ref_param = self.llm.h[0].ln_1.weight
            elif hasattr(self.llm, "base_model") and hasattr(self.llm.base_model, "h"): # PEFT wrapping
                ref_param = self.llm.base_model.h[0].ln_1.weight
            elif hasattr(self.llm, "model") and hasattr(self.llm.model, "h"): # AutoModel wrapping
                ref_param = self.llm.model.h[0].ln_1.weight
            else:
...
class LLM4GNN(nn.Module):
    def __init__(self, args, supports=None):
        ...
        # 修改：默认路径为 GPT2
        self.llm_name = getattr(args, "llm_name", "/home/dragonfly/LLMs/gpt2")
        ...
        if self.use_llm:
            ...
            # 修改：实例化 GPT2Adapter
            self.gpt2_adapter = GPT2Adapter(
                model_name_or_path=self.llm_name,
                ts_dim=self.hid_dim,           
                lora_r=self.lora_r, 
                lora_alpha=16, lora_dropout=0.05,
                dtype=torch.bfloat16, # GPT2 usually FP32 but BF16 works fine if supported
                local_files_only=True, 
                enable_grad_ckpt=True
            )
            self.reg_head = RegressionHead(
                d_in=self.gpt2_adapter.llm_hidden_size, # 动态获取真实维度
    ...
    def forecasting(self, time_steps_to_predict, X, truth_time_steps, mask=None, return_aux_losses=False):
        ...
        q_hidden = self.gpt2_adapter(tokens, attn_mask, idx_ranges)
        ...
        h_final_up = F.linear(h_final, self.gpt2_adapter.input_proj.weight, self.gpt2_adapter.input_proj.bias)
