...
parser.add_argument('--topk', type=int, default=16,
                    help='Top-K neighbors per node per head for attention pruning (0 disables pruning).')
parser.add_argument('--dropout', type=float, default=0.1,
                    help='Dropout rate used in attention and FFN blocks.')
parser.add_argument('--time_decay_init', type=float, default=0.1,
                    help='Initial beta for time-decay kernel exp(-beta*|Δt|).')
parser.add_argument('--edge_topk', type=int, default=8,
                    help='每个节点每种关系最多的邻居数，-1 代表全连接（危险）。')
parser.add_argument('--time_window', type=float, default=-1,
                    help='仅连接 |Δt| <= window 的边；<=0 关闭窗口。')
...
from model.hipatch_plus import *
import warnings
